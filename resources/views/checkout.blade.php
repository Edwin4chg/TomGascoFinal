...
<html>
    ...
    <body data-bs-spy="scroll" data-bs-target="#navbar" data-bs-root-margin="0px 0px -40%" data-bs-smooth-scroll="true" tabindex="0" id="intro">
        ...
        <main style="margin-bottom: 10rem">
            <div class="container-lg">
                <h2 class="display-2 mt-5 mb-4 text-center">Tu Carrito</h2>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="userEmail" class="form-control" value="{{ auth()->user()->email }}" readonly>

                    <!-- Puedes agregar más campos según tu tabla de usuarios -->
                </div>

                <!-- Detalles del Carrito -->
                <div class="list-group mb-3">
                    <h4>Tu Pedido</h4>
                    @forelse($cartSongsDetails as $cartSong)
                        <div class="list-group-item border-0 d-flex justify-content-between lh-sm">
                            <div class="d-flex align-items-center">
                                <div class="image-holder text-center p-3 me-3 border rounded-4">
                                    <img src="{{ asset('storage/' . $cartSong['photo']) }}" alt="{{ $cartSong['title'] }}" class="img-fluid" style="width: 80px; height: 80px; object-fit: cover; border-radius: 7px;">
                                </div>
                                <div>
                                    <h6 class="my-0">{{ $cartSong['title'] }}</h6>
                                    <small class="text-body-secondary">{{ $cartSong['description'] }}</small>
                                </div>
                            </div>
                            <span class="text-body-secondary">${{ $cartSong['price'] }}</span>
                        </div>
                    ...
                    <li class="list-group-item border-0 d-flex justify-content-end">
                        <span class="fs-3">Total (USD)</span>
                        <strong class="ms-3 me-5 fs-3">${{ $totalPrice }}</strong>
                    </li>
                </div>
                </div>
                ...
                <div class="d-flex flex-column mt-4">
                    <a style="width: auto; margin-left:auto; margin-right:auto;" class="btn btn-danger btn-lg" href="{{ url('/cart/empty') }}"> Vaciar Carrito</a>
                    <div id="paypal-button-container" class="hidden p-4 bg-gray-100">
            ...
            <div class="container">
                ...
                <div class="col-lg-3 col-md-6">
                    ...
                    <ul class="menu-list list-unstyled">
                        ...
                        <li class="menu-item pb-2">
                        <a href="#intro" class="item-anchor">Tienda</a>
                        </li>
                        <li class="menu-item pb-2">
            ...
            paypal.Buttons({
                ...
                createOrder: function (data, actions) {
                    return actions.order.create({
                        ...
                        payer: {
                            email_address: '{{$userEmail}}', // Usar Blade para imprimir la variable
                            name: {
                                given_name: '{{$namee}}',
                                surname: '{{$lastname}}',
                            },
                            address: {
                                postal_code: '111111',
                                country_code: 'PE' // Puedes establecer un valor predeterminado o modificar según tus necesidades
                            },
                        },
                        purchase_units: [{
                            amount: {
                ...
                onApprove: function (data, actions) {
                    // Capture the order to finalize the transaction
                    return actions.order.capture().then(function (orderData) {
                        // Access the transaction information
                        const transaction = orderData.purchase_units[0].payments.captures[0];

                        // Perform additional actions based on the transaction status
                        if (transaction.status === 'COMPLETED') {
                            // The transaction was completed successfully
                            console.log(orderId); // Aquí puedes acceder al orderId en tu script
                            window.location.href = "/checkout/process/" + orderId;
                        } else {
                            // The transaction was not completed successfully
                            alert('Transaction was not completed successfully! Transaction status: ' + transaction.status);
                        }
                    });
                }
